...
let arr_cv: Array<HTMLCanvasElement> = [];
...
async function main() {
  ...
  let a: unknown = await invoke("get_frame", { num: 10 });
...
interface item {
  ...
  pos_left: number;
  pos_right: number;
  getFrame(info: frame_args): frame;
}

interface frame_args {}
interface info {}

interface video_args extends frame_args {
  frame_num: number;
}

interface video_info extends info {
  filename: string;
  width: number;
  height: number;
}

interface video_item extends item {
  getInfo(): video_info;
  getFrame(args: video_args): frame;
}
